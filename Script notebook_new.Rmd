...
Fit the data to a linear line, using a total least squares regression and create a plot with the equation.

```{r}
df = data.frame(y=merge_ALL$RT_long, x=merge_ALL$RT_short)
pca <- prcomp(~x+y, df)
slp <- with(pca, rotation[2,1] / rotation[1,1])
int <- with(pca, center[2] - slp*center[1])

plot = ggplot(merge_ALL, aes(x=RT_short,y=RT_long))+geom_point()+
 geom_abline(slope=slp, intercept=int, color="blue", linewidth=1)+
  annotate("text",x=6,y=30,label=paste("y=",signif(slp,digits=4)," x + (",signif(int,digits=4),")"))
...
PLGSlong_newRT$precursor.retT=(PLGSlong_newRT$precursor.retT-int)/slp
